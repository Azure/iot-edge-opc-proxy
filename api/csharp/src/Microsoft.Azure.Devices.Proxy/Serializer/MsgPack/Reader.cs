...
namespace Microsoft.Azure.Devices.Proxy.MsgPack {
    ...
    public class MsgPackReader : IDisposable {
        ...
        /// <summary>
        /// Last integer value or length of array/raw/map
        /// </summary>
        public ulong Value { get; private set; }

        /// <summary>
        /// Last floating point value
        /// </summary>
        ...
        public MsgPackReader(Stream strm, bool owner = false) {
            _strm = strm;
            _owner = owner;
        }
        ...
        public long Signed {
            get {
                switch(_type) {
                    ...
                    case Types.Int8:
                        return (long)Value | (0xffffffffffffff << 8);
                    case Types.Int16:
                        return (long)Value | (0xffffffffffff << 16);
                    case Types.Int32:
                        return (long)Value | (0xffffffff << 32);
                    case Types.Int64:
                    default:
                        return (long)Value;
                }
        ...
        public async Task ReadAsync(CancellationToken ct) {
            ...
            switch (_type) {
                case Types.Float:
                    await ReadAsync(_buffer, 4, ct);
                    if (BitConverter.IsLittleEndian) {
                        _buffer[ 4] = _buffer[ 3];
                        _buffer[ 5] = _buffer[ 2];
                        _buffer[ 6] = _buffer[ 1];
                        _buffer[ 7] = _buffer[ 0];
                        Double = BitConverter.ToSingle(_buffer, 4);
                    }
                    else {
                        Double = BitConverter.ToSingle(_buffer, 0);
                    }
                    break;
                case Types.Double:
                    await ReadAsync(_buffer, 8, ct);
                    if (BitConverter.IsLittleEndian) {
                        _buffer[ 8] = _buffer[ 7];
                        _buffer[ 9] = _buffer[ 6];
                        ...
                        Double = BitConverter.ToDouble(_buffer, 8);
                    }
                    else {
                        Double = BitConverter.ToDouble(_buffer, 0);
                    }
                    break;
                case Types.Nil:
                    break;
        ...
        private async Task ReadAsync(byte[] buf, int len, CancellationToken ct) {
            int read = 0;
            while (read != len) {
                read += await _strm.ReadAsync(buf, read, len - read, ct);
            }
        }
